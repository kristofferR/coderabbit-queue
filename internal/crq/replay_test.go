...
func TestReplayRateLimitBounceFiresOncePerWindow(t *testing.T) {
	...
	expectedRetry := base.Add(40*time.Minute + 30*time.Second) // comment time plus countdown and safety margin
	...
	for _, elapsed := range []time.Duration{40 * time.Minute, 40*time.Minute + 29*time.Second} {
...
func TestReplay448DaySequenceFiresThreeTimes(t *testing.T) {
	...
	f.pump()
	if r := f.round(repo, pr); r == nil || r.Phase != PhaseAwaitingRetry ||
		r.RetryAt == nil || !r.RetryAt.Equal(base.Add(57*time.Minute+30*time.Second)) {
		t.Fatalf("the parseable 40m window must park until base+57m30s, got %#v", r)
	}
	...
	f.clk.set(base.Add(57*time.Minute + 30*time.Second))
