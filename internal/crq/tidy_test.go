...
func TestTidyRemovesOnlySpentCommandsCrqPosted(t *testing.T) {
	...
	add(300, "someone-else", cfg.ReviewCommand, now.Add(-3*time.Hour))
	...
	// One archived round holding the spent command, one live round holding 200.
	if _, err := store.Update(ctx, func(st *State) error {
		old, err := st.NewRound(repo, pr, "aaaaaaaa1", now.Add(-3*time.Hour))
		if err != nil {
			return err
		}
	...
	if !left[300] {
		t.Error("a person's review request was deleted; that was not crq's to erase")
	}
	if !left[400] {
