...
// fireCoOnly handles a round whose primary is already finished or disabled but
// whose gating co-reviewers have not answered. It posts ONLY their trigger
// commands and records the round as fired with the first trigger as the
// CommandID anchor. No primary quota is spent and therefore NO FireSlot is
// taken: the per-round trigger claims (CoBots[login].ClaimedAt, CAS-set before
// the network post) are the concurrency guard.
func (s *Service) fireCoOnly(ctx context.Context, cfg Config, round Round, logins []string, reason string, now time.Time) (PumpResult, error) {
	...
	s.sync(ctx, updated)
	if s.log != nil {
		s.log.Printf("fire %s@%s (%s; posted co-reviewer triggers for %s)", key, round.Head, reason, strings.Join(claimed, ","))
	}
	return PumpResult{Action: "fired", Repo: round.Repo, PR: round.PR, Head: round.Head, Reason: reason}, nil
}
