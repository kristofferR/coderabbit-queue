...
export function OverviewPage({
  ...
  const run = async (reason: string) => {
    ...
    setError(null);
    try {
      const { snapshot } = await act(pending.kind, { repo: pending.repo, pr: pending.pr, reason });
      onSnapshot?.(snapshot);
      setPending(null);
    } catch (e) {
    ...
    <main className="mx-auto max-w-[1400px] px-6 pt-4.5 pb-16">
      ...
      {ov.attention.map((a, i) => (
        <div
          ...
          className={`mb-3.5 flex flex-wrap items-center gap-3 rounded-[10px] border border-l-4 px-4 py-2.5 text-[13.5px] ${
            a.level === "bad" ? "border-bad-edge border-l-bad bg-bad-bg" : "border-warn-edge border-l-warn-fg bg-warn-bg"
          }`}
        >
      ...
      >
        ...
        <div className="flex flex-wrap gap-3 px-[18px] py-3">
          {ov.autofix.hosts.length === 0 && <span className="text-[13px] text-faint">No host has reported yet.</span>}
      ...
      {pending && (
        <Confirm
          ...
          body={
            pending.kind === "hold" ? (
              <>crq will stop reviewing this pull request until the hold is lifted. Nothing already in
              flight is undone.</>
            ) : pending.kind === "unhold" ? (
              ...
              <>
                ...
                {pending.slot && (
                  <>
                    {" "}
                    <b className="text-bad">This round holds the fire slot</b>, so cancelling releases
                    it for the next PR.
                  </>
                )}
              </>
          ...
          confirmLabel={pending.kind === "hold" ? "Hold it" : pending.kind === "unhold" ? "Unhold" : "Cancel the round"}
 ...
 */
function ActivityFeed({ events, now }: { events: EventItem[]; now: number }) {
...
function RowActions({ onHold, onCancel }: { onHold: () => void; onCancel: () => void }) {
  return (
    <span className="flex justify-end gap-2.5 whitespace-nowrap opacity-0 transition-opacity group-hover:opacity-100">
      ...
      <button type="button" onClick={onCancel} className="text-[12.5px] text-acc hover:underline">
        Cancel
      </button>
    </span>
  );
  ...
  return (
    ...
    >
      <div className="max-[1150px]:basis-full">
        ...
        {ov.headline.detail && <div className="mt-0.5 text-[13px] text-mut">{ov.headline.detail}</div>}
